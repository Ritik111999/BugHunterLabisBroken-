...
@section('content')
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-xl font-semibold text-gray-800 dark:text-white/90">Analytics export</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">Export meeting analytics CSV with optional filters.</p>
        </div>
    </div>

    <form method="GET" action="{{ route('admin.meetings.export_analytics') }}"
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div class="text-sm text-gray-700 dark:text-gray-200">
                Download a CSV of meeting analytics matching the current filters.
            </div>
            <a href="{{ route('admin.meetings.export_analytics_download', request()->query()) }}"
